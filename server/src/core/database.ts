...
export class DatabaseManager {
    ...
    constructor(private readonly logger: Logger, private readonly dbBasePath: string = "db") {
        ...
        this.#configDB = new Low<ConfigDatabase>(
            new JSONFile(path.join(this.dbBasePath, DatabaseManager.configFilePath)),
            { configs: [] }
        );

        this.#credentialsDB = new Low<CredentialsDatabase>(
            new JSONFile(path.join(this.dbBasePath, DatabaseManager.credentialsFilePath)),
            {
                testValue: "",
    ...
    public async initialize(): Promise<boolean> {
        ...
        try {
            ...
            this.logger.setup.core.error(DatabaseManager.loggerComponentName, `An error happened during reading database files: ${error}`);
            return false;
        }
        ...
        if (!this.#credentialsDB.data.testValue) {
            this.logger.setup.core.info(DatabaseManager.loggerComponentName, "It seems like this is the first run of the database. Setting up default values.");
            this.#credentialsDB.data.testValue = this.encryptValue(DatabaseManager.testValue);
            try {
                await this.#credentialsDB.write();
            } catch (error) {
                this.logger.setup.core.error(DatabaseManager.loggerComponentName, `An error happened during writing of the test value: ${error}`);
                return false;
            }
        }
    ...
    private encryptValue(value: string): string {
        if (!this.#credentialsKey) {
            this.logger.run.core.error(DatabaseManager.loggerComponentName, "Credentials encryption initialization failed.");
            throw new Error("Credentials encryption initialization failed.");
        }

        const iv = crypto.randomBytes(16);
        const cipher = crypto.createCipheriv(DatabaseManager.encryptionAlgorithm, this.#credentialsKey, iv);
    ...
    private decryptValue(encryptedValue: string): string {
        if (!this.#credentialsKey) {
            this.logger.run.core.error(DatabaseManager.loggerComponentName, "Credentials decryption initialization failed.");
        ...
        const decipher = crypto.createDecipheriv(DatabaseManager.encryptionAlgorithm, this.#credentialsKey, iv);
        const decrypted = Buffer.concat([decipher.update(encryptedData), decipher.final()]);
        return decrypted.toString('utf-8');
    }

    private async askForPassword(): Promise<void> {
        this.logger.setup.core.debug(DatabaseManager.loggerComponentName, "Asking user for credentials database password now.");
    ...
    public async setCredential(serviceName: string, key: string, value: string): Promise<void> {
        ...
        try {
            await this.#credentialsDB.write();
            this.logger.run.core.debug(DatabaseManager.loggerComponentName, "Credentials saved successfully.");
        } catch (error) {
            this.logger.run.core.error(DatabaseManager.loggerComponentName, `An error happened during saving credential: ${error}`);
        }
    }
    ...
    public async setConfig(tileName: string, key: string, value: string): Promise<void> {
        ...
        try {
            ...
            this.logger.run.core.debug(DatabaseManager.loggerComponentName, "Configs saved successfully.");
        } catch (error) {
            this.logger.run.core.error(DatabaseManager.loggerComponentName, `An error happened during saving config: ${error}`);
        }
    }
}
